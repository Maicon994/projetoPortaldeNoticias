...
if ($logado) {
    // Exibe apenas as notícias do próprio usuário
    $stmt = $pdo->prepare("
    ...
    // Deslogado: Exibe todas as notícias do portal
    $stmt = $pdo->query("
        SELECT n.*, u.nome as autor_nome,
        (SELECT COUNT(*) FROM likes_noticia WHERE noticia_id = n.id) as total_likes,
        (SELECT COUNT(*) FROM comentarios WHERE noticia_id = n.id) as total_comentarios
...
<div class="container">
    <h2><?= $logado ? "📰 Suas Notícias Postadas" : "🌍 Últimas do Mundo Inteiro (Fake)" ?></h2>
